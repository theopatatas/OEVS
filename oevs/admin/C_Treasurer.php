...
<head>
  <meta charset="UTF-8">
  <title>Treasurer - Canvassing Report</title>
  ...
  <style>
    ...
    .nav-item>a{color:var(--primary-color);font-weight:600;padding:8px 12px;border-radius:6px;display:inline-block;text-decoration:none;transition:var(--transition)}
    ...
    /* ===== Toolbar row (left-aligned, like screenshot) ===== */
    .filter-section{
      background:var(--white);
      padding:15px 20px;
      border-radius:10px;
  ...
  <div class="content-wrapper">
    ...
    <div class="table-container">
      ...
      <table id="reportTable">
        ...
        <tbody>
          <?php
          $candidate_query = mysqli_query($conn, "SELECT * FROM candidate WHERE Position='Treasurer'");
    ...
    (function(){
      ...
      function applyPerPage(){
        ...
        Array.from(tbody.rows).forEach((r)=>{
          const isVisibleBySearch = r.dataset._searchMatch !== '0'; // set by applySearch
          if (!isVisibleBySearch) { r.style.display = 'none'; return; }
