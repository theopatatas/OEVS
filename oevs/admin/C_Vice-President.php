...
<head>
  <meta charset="UTF-8">
  <title>Vice-President - Canvassing Report - Online Voting System</title>
  ...
  <style>
    ...
    body { font-family: var(--font); background-color: var(--bg-color); margin: 0; padding: 0; color: #333; }
    ...
    .dropdown { display:none; position:absolute; top:100%; left:0; background:var(--white); box-shadow:var(--shadow); border-radius:6px; min-width:200px; padding:8px 0; z-index:99; white-space:nowrap; }
    .dropdown a { display:block; padding:10px 15px; color:var(--primary-color); text-decoration:none; font-weight:500; transition:var(--transition); }
    ...
    .filter-dropdown-menu {
      display:none; position:absolute; background:var(--white); box-shadow:var(--shadow);
      border-radius:6px; min-width:220px; padding:8px 0; z-index:10; margin-top:8px;
    }
    /* click-to-open */
    .filter-dropdown.open .filter-dropdown-menu { display:block; }

    .filter-dropdown-menu a { display:flex; align-items:center; gap:10px; padding:10px 15px; color:var(--primary-color); text-decoration:none; font-weight:500; }
    ...
    .items-per-page select { padding:8px 12px; border:1px solid #ddd; border-radius:6px; background:#fff; }
    .search-box { display:flex; align-items:center; gap:10px; font-size:14px; color:#666; }
    .search-box input { padding:8px 15px; border:1px solid #ddd; border-radius:6px; min-width:250px; }

    table { width:100%; border-collapse:collapse; min-width:800px; }
    thead { background: linear-gradient(135deg, var(--primary-color) 0%, var(--accent-color) 100%); }
    th { padding:16px; text-align:left; font-weight:600; color:#fff; font-size:14px; text-transform:uppercase; letter-spacing:0.5px; }
    td { padding:16px; border-bottom:1px solid #f0f0f0; font-size:14px; color:#333; vertical-align:middle; }
    ...
    .btn { padding:8px 14px; border:none; border-radius:6px; cursor:pointer; font-size:13px; font-weight:600; display:inline-flex; align-items:center; gap:6px; }
...
<body>
  ...
  <header>
    ...
    <nav>
      ...
      <!-- Menu dropdown (from home.php) -->
      <?php
      if (file_exists('nav_menu_dropdown.php')) {
          include('nav_menu_dropdown.php');
      } else {
      ?>
      <div class="nav-item">
  ...
  <div class="content-wrapper">
    ...
    <!-- Toolbar row: Filter • Add • Download (same line, left) -->
    <div class="filter-section">
      ...
      <button class="add-btn" onclick="window.location.href='add_candidate.php'"><i class="fas fa-plus"></i> Add Candidates</button>
    ...
    <div class="table-container">
      <div class="table-controls">
        <div class="items-per-page">
          ...
          <select id="itemsPerPage"><option value="15">15</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select>
        </div>
      ...
      <table id="reportTable">
        ...
        <tbody>
          <?php
          // Filtered query: Vice-President
          $candidate_query = mysqli_query($conn, "SELECT * FROM candidate WHERE Position='Vice-President'");
          while ($candidate_rows = mysqli_fetch_array($candidate_query)) {
            $id = $candidate_rows['CandidateID'];
          ?>
          <tr>
            <td><?php echo htmlspecialchars($candidate_rows['Position']); ?></td>
            <td><?php echo htmlspecialchars($candidate_rows['Party']); ?></td>
            <td><?php echo htmlspecialchars($candidate_rows['FirstName']); ?></td>
            <td><?php echo htmlspecialchars($candidate_rows['LastName']); ?></td>
            <td><?php echo htmlspecialchars($candidate_rows['Year']); ?></td>
            <td>
              <img class="candidate-photo" src="<?php echo htmlspecialchars($candidate_rows['Photo']);?>"
                   alt="<?php echo htmlspecialchars($candidate_rows['FirstName']." ".$candidate_rows['LastName']);?>"
                   title="<?php echo htmlspecialchars($candidate_rows['FirstName']." ".$candidate_rows['LastName']);?>">
            </td>
            <td style="text-align:center;">
              <?php
                $votes_query = mysqli_query($conn, "SELECT 1 FROM votes WHERE CandidateID='$id'");
                echo mysqli_num_rows($votes_query);
              ?>
            </td>
          </tr>
          <?php } ?>
  ...
  <!-- Search + items-per-page cooperate -->
  <script>
    (function(){
      ...
      function applyPerPage(){
        ...
        Array.from(tbody.rows).forEach((r)=>{
          const match = r.dataset._searchMatch !== '0';
          if (!match) { r.style.display = 'none'; return; }
      ...
      function applySearch(){
        ...
        Array.from(tbody.rows).forEach((r)=>{
          ...
          r.dataset._searchMatch = ok ? '1' : '0';
        });
        setTimeout(applyPerPage, 0);
      }

      if (searchInput) searchInput.addEventListener('input', applySearch);
      if (itemsSelect) itemsSelect.addEventListener('change', applyPerPage);
