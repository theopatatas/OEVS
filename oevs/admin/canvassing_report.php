...
/* current file for active filter state */
$self = basename($_SERVER['PHP_SELF']);

function _flash_redirect($ok, $msg) {
  if (session_status() !== PHP_SESSION_ACTIVE) { session_start(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
  if (empty($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
    _flash_redirect(0, 'Invalid request. Please try again.');
  }

  $id = (int)$_POST['delete_id'];
  if ($id <= 0) _flash_redirect(0, 'Invalid candidate id.');

  // Get candidate for message and photo cleanup
  if (!$stmt = mysqli_prepare($conn, "SELECT FirstName, LastName, Photo FROM candidate WHERE CandidateID = ?")) {
    _flash_redirect(0, 'Database error (prep).');
  }
  ...
  if (!$cand) _flash_redirect(0, 'Candidate not found.');

  // Delete dependent votes first (MyISAM has no FK cascade)
  if ($d1 = mysqli_prepare($conn, "DELETE FROM votes WHERE CandidateID = ?")) {
  ...
  if ($affected > 0) {
    ...
    _flash_redirect(1, 'Candidate deleted: '.$cand['FirstName'].' '.$cand['LastName']);
  } else {
    _flash_redirect(0, 'Delete failed.');
  }
}
?>
...
<head>
  ...
  /* ===== Content ===== */
  .content-wrapper{ max-width:1400px; margin:0 auto; padding:35px 20px 0; }
  ...
  .table-controls{ padding:20px; display:flex; justify-content:space-between; align-items:center; border-bottom:2px solid #f0f0f0; flex-wrap:wrap; gap:15px; background:#fafbfc }
  .items-per-page{ display:flex; align-items:center; gap:10px; font-size:14px; color:#666 }
  .items-per-page select{ padding:8px 12px; border:1px solid #ddd; border-radius:6px; font-size:14px; background:#fff; cursor:pointer; transition:var(--transition) }
  ...
  th{ padding:16px; text-align:left; font-weight:600; color:#fff; font-size:14px; text-transform:uppercase; letter-spacing:.5px }
  ...
  .btn-edit:hover{ background:var(--accent-color); transform:translateY(-2px); box-shadow:0 6px 12px rgba(0,86,179,.3) }
  .btn-view{ background:var(--accent-color); color:#fff }
  .btn-view:hover{ background:#003d82; transform:translateY(-2px); box-shadow:0 6px 12px rgba(0,47,108,.3) }
  .btn-delete{ background:#dc3545; color:#fff }
  .btn-delete:hover{ background:#bd2130; transform:translateY(-2px); box-shadow:0 6px 12px rgba(220,53,69,.3) }

  /* Download Excel = green, NO shadow */
  .btn-success{
  ...
  .modal{ background:#fff; width:min(880px,96vw); border-radius:12px; box-shadow:var(--shadow); overflow:hidden }
  .modal-header{ display:flex; align-items:center; justify-content:space-between; padding:14px 18px; background:linear-gradient(135deg,var(--primary-color) 0%,var(--accent-color) 100%); color:#fff }
...
<body>
  ...
  <div class="content-wrapper">
    ...
    <!-- Toolbar row (updated to pill filter) -->
      ...
      <table id="candidateTable">
        ...
        <tbody>
          ...
          while($candidate_rows = mysqli_fetch_array($candidate_query)) { 
            ...
            // compute votes once (also used by modal)
          ...
          <tr class="del<?php echo $id ?>">
            ...
            <td>
              <div class="action-buttons">
                <button class="btn btn-edit" onclick="window.location.href='edit_candidate.php?id=<?php echo $id; ?>'">
                ...
                <button
                  ...
                  data-id="<?php echo $id; ?>"
                  data-firstname="<?php echo $esc($candidate_rows['FirstName']); ?>"
                  data-middlename="<?php echo $esc($candidate_rows['MiddleName']); ?>"
                  data-lastname="<?php echo $esc($candidate_rows['LastName']); ?>"
                  data-gender="<?php echo $esc($candidate_rows['Gender']); ?>"
                  data-year="<?php echo $esc($candidate_rows['Year']); ?>"
                  data-position="<?php echo $esc($candidate_rows['Position']); ?>"
                  data-party="<?php echo $esc($candidate_rows['Party']); ?>"
                  data-photo="<?php echo $esc($candidate_rows['Photo']); ?>"
                  data-qualification="<?php echo $esc($candidate_rows['Qualification']); ?>"
                ...
                <form method="POST" style="display:inline;"
                      onsubmit="return confirm('Are you sure you want to delete this candidate? This will also remove their votes and photo.');">
      ...
      <div class="modal-body">
        <div class="modal-grid">
          ...
          <div>
            ...
            <div class="field"><label>Position</label><div class="value" id="vmPosition">—</div></div>
  ...
  <script>
    (function () {
      ...
      function setRowVisible(row, visible) { row.style.display = visible ? '' : 'none'; }

      function filterRows(query) {
        const q = query.trim().toLowerCase();
        const rows = Array.from(tbody.rows);
        if (!q) { rows.forEach(r => setRowVisible(r, true)); return; }
        rows.forEach(row => setRowVisible(row, row.textContent.toLowerCase().includes(q)));
      }

      let t = null;
      function debouncedFilter() { clearTimeout(t); t = setTimeout(() => filterRows(searchInput.value), 120); }
  ...
  <script>
    (function () {
      ...
      function openFrom(btn) {
        ...
        $('vmParty').textContent = ds.party || '—';
        $('vmPosition').textContent = ds.position || '—';
        $('vmGender').textContent = ds.gender || '—';
        $('vmYear').textContent = ds.year || '—';
        $('vmVotes').textContent = ds.votes || '0';
        $('vmQual').textContent = ds.qualification || '—';
