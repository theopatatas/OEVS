  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .nav-item > a{
      ...
      font-weight:700; color:var(--primary); transition:all .2s ease;
    }
    ...
    .dropdown, .submenu{
      ...
      background:#fff; border:1px solid #e7eef7; border-radius:14px;
    ...
    .dropdown a, .submenu a{
      ...
      color:var(--primary); font-weight:600; white-space:nowrap; transition:background .16s ease, color .16s ease;
    }
    .dropdown a:hover, .submenu a:hover, .dropdown a:focus-visible, .submenu a:focus-visible{background:var(--accent); color:#fff; outline:none}
    .dropdown .divider{height:1px; background:#e9eff7; margin:6px 4px}

    /* Admin Actions submenu (hover) */
    .has-submenu{position:relative}
    .submenu{position:static; top:auto; left:auto; min-width:auto; border:none; box-shadow:none; padding:4px 0 0 0; margin:0}
    ...
    .card{
      ...
      padding:14px; overflow:visible; /* avoid clipping filter dropdown */
    }
    ...
    .btn{
      ...
      background:#f7f9fc; color:#0b1b36; font-weight:700; border-radius:10px; padding:9px 12px; cursor:pointer;
    }
    ...
    .filter-menu{
      ...
      border-radius:12px; box-shadow:var(--shadow); min-width:220px; padding:6px; z-index:2000;
    }
    .filter-menu a{
      display:flex; align-items:center; gap:10px; padding:10px 12px; color:var(--primary); border-radius:10px; font-weight:600;
    }
    .filter-menu a:hover{background:var(--accent); color:#fff}

    /* Search + page size */
    .control{display:flex; align-items:center; gap:10px}
    .control label{font-weight:600; color:#163055}
    select, input[type="search"]{
      height:36px; border:1px solid var(--border); border-radius:10px; padding:0 10px; font:inherit; outline:none;
    }
    ...
    .badge{
      ...
      background:#eef2f7; color:#344a6a; border:1px solid #e1e8f5;
    }
    .badge-success{background:var(--success); color:var(--success-ink); border-color:#bfe9cf}
    .badge-muted{background:#eef2f7; color:#596a84; border-color:#e1e8f5}

    /* Footer row (pagination) */
    .table-footer{
      display:flex; align-items:center; justify-content:flex-end; gap:10px; padding:12px 8px 4px;
      color:#445c7a; font-size:14px;
    }
    .pager{display:inline-flex; align-items:center; gap:6px}
    .pager button{
      width:34px; height:34px; border:1px solid var(--border); border-radius:8px; background:#fff; cursor:pointer;
    }
    .pager button:hover{background:#f1f6ff}

    footer{text-align:center; padding:20px 0; color:#6c7b90; font-size:14px}
        ...
        <div class="left-actions">
          <div class="filter-wrap">
            <button class="btn" id="filterBtn" type="button">
              <i class="fa fa-filter"></i> Filter By Position
              <i class="fa fa-caret-down" aria-hidden="true"></i>
            </button>

<div class="filter-menu" id="filterMenu">
  <a href="voter_list.php"><i class="fa fa-table"></i> All</a>
  <a href="Voted_voters.php"><i class="fa fa-circle-check"></i> Voted Voters</a>
  <a href="Unvoted_voters.php"><i class="fa fa-circle"></i> Unvoted Voters</a>
</div>
          ...
          <tbody id="tableBody">
            <?php
              ...
              while ($voter_rows = mysqli_fetch_array($voter_query)):
                ...
                $status = trim($voter_rows['Status']);
                $isVoted = strcasecmp($status,'voted')===0 || $status==='1' || strcasecmp($status,'yes')===0;
            ?>
            <tr class="del<?php echo $id; ?>">
              ...
              <td>
                ...
                <?php else: ?>
                  <span class="badge badge-muted">Unvoted</span>
                <?php endif; ?>
              </td>
              <td>
                <a class="btn btn-delete" data-id="<?php echo $id; ?>"
                   style="background:#ffe8ea;border-color:#ffd1d6;color:#b61e1e;gap:6px">
...
<script>
  ...
  (function(){
    ...
    document.addEventListener('click', (e)=>{
      if(!e.target.closest('.filter-wrap')) menu.style.display='none';
    });
  })();
  ...
  // Delete AJAX — only for .btn-delete
  $(function(){
    const d = new Date();
