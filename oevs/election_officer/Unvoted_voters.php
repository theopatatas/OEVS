...
<head>
  <meta charset="UTF-8" />
  <title>Unvoted Voters - Online Election Voting System</title>
  ...
  <style>
    :root{
      ...
      --table-head:#0a2e5c; --danger:#ef4351;
      --badge:#eef2f7; --badge-ink:#344a6a; --badge-br:#e1e8f5;
      --badge-muted:#f1f2f6; --badge-muted-ink:#596a84; --badge-muted-br:#e5e9f4;
    }
    *{box-sizing:border-box}
    ...
    .card{
      ...
      padding:14px; overflow:visible; /* allow filter dropdown to spill out */
    }

    /* Toolbar in card header */
    .toolbar{display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap; padding:6px 6px 10px}
    ...
    .filter-menu{display:none; position:absolute; top:calc(100% + 6px); left:0; background:#fff; border:1px solid var(--border); border-radius:12px; box-shadow:var(--shadow); min-width:240px; padding:6px; z-index:2000}
    ...
    .badge{display:inline-flex; align-items:center; gap:6px; padding:6px 10px; border-radius:999px; font-weight:700; font-size:12px;
      background:var(--badge); color:var(--badge-ink); border:1px solid var(--badge-br)}
    .badge-muted{background:var(--badge-muted); color:var(--badge-muted-ink); border-color:var(--badge-muted-br)}
  ...
  <a href="Unvoted_voters.php"><i class="fa fa-circle"></i> Unvoted Voters</a>
      ...
      <div class="demo_jui">
        <table id="votersTable">
          ...
          <tbody id="tableBody">
            <?php
              $voter_query = mysqli_query($conn, "SELECT * FROM voters WHERE Status='UnVoted'");
            ...
            <tr class="del<?php echo $id; ?>">
              ...
              <td><span class="badge badge-muted">Unvoted</span></td>
              <td>
                <a class="btn-danger btn-delete" data-id="<?php echo $id; ?>">
                  <i class="fa fa-trash"></i> Delete
                </a>
                <input type="hidden" class="data_name<?php echo $id; ?>" value="<?php echo htmlspecialchars($voter_rows['FirstName'].' '.$voter_rows['LastName']); ?>"/>
                <input type="hidden" class="user_name" value="<?php echo htmlspecialchars($_SESSION['User_Type']); ?>"/>
              </td>
            </tr>
            <?php endwhile; ?>
          </tbody>
...
<input type="hidden" class="pc_date" />
<input type="hidden" class="pc_time" />
