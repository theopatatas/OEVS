...
function nextPosition(currentIndex) {
    ...
    if (next) { next.style.display = 'block'; next.scrollIntoView({ behavior: "smooth" }); }
}
function previousPosition(currentIndex) {
    document.getElementById('position_' + currentIndex).style.display = 'none';
    var prev = document.getElementById('position_' + (currentIndex - 1));
    if (prev) { prev.style.display = 'block'; prev.scrollIntoView({ behavior: "smooth" }); }
}
...
window.onload = function() {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (container) {
            ...
            const maxScroll = container.scrollWidth - container.clientWidth;
            if (maxScroll <= 0) { document.getElementById('right_' + i).disabled = true; }
        }
    }
};

// Countdown Timer
var timeLeft = <?php echo max(0, $time_limit - $elapsed_time); ?>;
...
function updateTimer() {
    if (timeLeft <= 0) {
        ...
        window.location.reload(); // Refresh to trigger PHP expiry check
    } else {
